...
class FraudLabsPro_WP_ECommerce {
	...
	public function admin_options() {
		if (is_admin()) {
			...
			<form method="post">
				...
				<p>If you like this plugin, please leave us a <a href="https://wordpress.org/support/view/plugin-reviews/fraudlabs-pro-for-wp-e-commerce">5 stars rating</a>. Thank You!</p>
			</div>';
		}
	}
	...
	public function screen_order(&$merchant_instance) {
		...
		$request = array(
			...
			'bill_addr'			=> $merchant_instance->cart_data['billing_address']['address'],
			'first_name'		=> $merchant_instance->cart_data['billing_address']['first_name'],
			'last_name'			=> $merchant_instance->cart_data['billing_address']['last_name'],
			...
			'user_phone'		=> $merchant_instance->cart_data['billing_address']['phone'],
			...
			'email'				=> $merchant_instance->cart_data['email_address'],
	...
	public function render_fraud_report() {
		...
		if (isset($_POST['reject_blacklist'])) {
			$request = array(
				'key' => get_option('fraudlabspro_api_key') ,
		...
		if (count($result) > 0) {
			...
			$table.= '
				<tr>
					<td rowspan="3">
						...
						<img class="img-responsive" alt="" src="//fraudlabspro.hexa-soft.com/images/fraudscore/' . (($row->fraudlabspro_score) ? 'fraudlabsproscore' . $row->fraudlabspro_score . '.png' : 'nofraudprotection.png') . '" style="width:85%;" /></center>
					</td>
					<td>
			...
			$table.= '
				...
				<tr>
					<td rowspan="3">
						...
						<p style="color:#' . $color . ';font-size:3.5em;font-weight:bold">' . $row->fraudlabspro_status . '</p></center>
					</td>
					<td>
			...
			if ($row->fraudlabspro_status == 'REVIEW') {
				...
				<form method="post">
					...
					<input type="hidden" name="flpnote" id="flpnote" >
					<input type="submit" name="approve" id="approve-order" value="Approve" style="padding:10px 5px; background:#22aa22; border:1px solid #ccc; min-width:100px; cursor: pointer; color: #fff;" />
					<input type="submit" name="reject" id="reject-order" value="Reject" style="padding:10px 5px; background:#cd2122; border:1px solid #ccc; min-width:100px; cursor: pointer; color: #fff;" />
					<input type="submit" name="reject_blacklist" id="reject_blacklist" value="Blacklist" style="padding:10px 5px; background:#cb5a5e; border:1px solid #ccc; min-width:100px; cursor: pointer; color: #fff;" onclick="submitfunction()"/>
					</p>
				</form>';
			}
			echo '
